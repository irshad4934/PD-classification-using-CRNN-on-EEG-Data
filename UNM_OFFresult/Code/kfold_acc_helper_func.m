function [acc_Kfold,all_scores,all_classes]=kfold_acc_helper_func(SelectedChans,YWTr_all,TrainingLabels,TrainingClasses,YWTest_all,AllParams,best_comb,is_normalized_projection,k)
...
for fold=1:total_fold
    ...
    for ch=1:length(SelectedChans)
        ...
        [KTrYWData,KTrainingLabels,KTrainingClasses,~,~,~]=training_test_get_fold(YWTraining,TrainingLabels,TrainingClasses,KFoldData,fold);
        [~,~,~,KYWTestData,KTestLabels,KTestClasses]=training_test_get_fold(YWTest,TrainingLabels,TrainingClasses,KFoldData,fold);
