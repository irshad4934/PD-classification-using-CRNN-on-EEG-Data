function [Aout,Eout,Kout] = arburg_ongpu(x, p)
...
validateattributes(gather(x),{'numeric'},{'nonempty','finite','2d'},'arburg','X');
...
N  = gpuArray(size(x,1));
Nchan = gpuArray(size(x,2));

% By convention all polynomials are row vectors
Aout = gpuArray(zeros(Nchan, p+1, class(gather(x))));
% Optional outputs
Eout = gpuArray(zeros(1, Nchan, class(gather(x))));
Kout = gpuArray(zeros(p, Nchan, class(gather(x))));
...
for ichan = 1:Nchan
  ...
  a =gpuArray( zeros(1,p+1,class(gather(x))));
  a(1) = 1;
