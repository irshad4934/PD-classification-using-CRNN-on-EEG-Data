...
cd(current_location);
cd './Organized_data';
save('EEG_Jim_rest_Unsegmented_WithAllChannelsEYESCLOSED.mat','EEG','Channel_location','-v7.3');
disp('Data saved !');
cd(current_location);
...
function Data=EyesOpenData(EEG)
    ...
    flag=0;
    for i=1:n
        if (strcmp( Allevents(i).type,'S  3' ) || strcmp( Allevents(i).type,'S  4' )) && flag ==0
            flag=1;
            startTime=Allevents(i).latency;
        end
        if (strcmp( Allevents(i).type,'S  3' ) || strcmp( Allevents(i).type,'S  4' )) && flag ==1
