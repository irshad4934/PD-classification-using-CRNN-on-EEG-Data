...
for condition=1:2
    for i=1:length(subjects{condition})
        ...
        elseif size(subject_index_in_ON_OFF_indicator,1)> 0
            Temp_session= ON_OFF_indicator(subject_index_in_ON_OFF_indicator,2) ; % get the ON session data
            % now ,find OFF session
            if Temp_session==1
                Temp_session=2;
            else
                Temp_session=1;
            ...
            Current_session=num2str(  Temp_session );  
        end
        %load file
        filename=[subjects{condition}{i},'_',Current_session,'_PD_REST.mat']; % only session 1 for now
        load(filename) ;
...
cd(current_location);
cd './Organized_data';
save('EEG_Jim_rest_OFF_Unsegmented_WithAllChannels.mat','EEG','Channel_location','-v7.3');
disp('Data saved !');
cd(current_location);